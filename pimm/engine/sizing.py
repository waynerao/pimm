...
def compute_optimal_quotes(df, max_buy_notional, max_sell_notional):
    ...
    result = df[["buy_raw", "sell_raw", "alpha", "stock_limit",
                  "last_price", "fx_rate", "lot_size"]].copy()
    ...
    buy_scaling = (
        min(1.0, max_buy_notional / buy_notional)
        if buy_notional > 0 else 1.0
    )
    sell_scaling = (
        min(1.0, max_sell_notional / sell_notional)
        if sell_notional > 0 else 1.0
    )
...
def apply_inventory_constraint(optimal_df, universe_df):
    ...
    dispatch.rename(
        columns={"buy_optimal": "buy_dispatch", "sell_optimal": "sell_dispatch"},
        inplace=True,
    )
