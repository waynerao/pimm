    ...
    async def websocket_endpoint(ws: WebSocket):
        ...
        try:
            ...
            logger.info(
                ...
                f"({len(app.state.ws_clients)} remaining)"
            )
...
def snapshot_to_json(snap):
    ...
    for mname, df in snap.markets.items():
        ...
        for ric in df.index:
            ...
            sent_str = (
                sent.strftime("%H:%M:%S")
                if pd.notna(sent) else ""
            )
            pnl_local = (
                r["last_price"]
                * (r["pnl_buy_qty"] - r["pnl_sell_qty"])
            ...
            records.append({
                ...
                "ratio": float(ratio),
                "filled_buy": float(
                    r["filled_buy_since_dispatch"]
                ),
                "filled_sell": float(
                    r["filled_sell_since_dispatch"]
                ),
                "last_sent": sent_str,
                "pnl_buy_qty": float(r["pnl_buy_qty"]),
                "pnl_sell_qty": float(r["pnl_sell_qty"]),
            })
        markets[mname] = records

    scaling = {}
    for mname, (bs, ss) in snap.scaling.items():
        scaling[mname] = {"buy": bs, "sell": ss}

    fills = []
    for f in snap.recent_fills:
        fills.append({
            "ric": f.ric, "side": f.side.value,
            "qty": f.fill_qty, "price": f.fill_price,
            "time": (
    ...
    market_configs = {}
    for mname, cfg in snap.market_configs.items():
        sessions_str = ", ".join(
            f"{s.start_hour:02d}:{s.start_minute:02d}-"
            f"{s.end_hour:02d}:{s.end_minute:02d}"
            for s in cfg.sessions
        )
        market_configs[mname] = {
            "sessions": sessions_str,
            "order_valid_time": cfg.order_valid_time,
            "refresh_buffer": cfg.refresh_buffer,
            "full_batch_interval": cfg.full_batch_interval,
            "min_dispatch_interval": (
                cfg.min_dispatch_interval
            ),
            "single_name_cap": cfg.single_name_cap,
            "max_buy_notional": cfg.max_buy_notional,
            "max_sell_notional": cfg.max_sell_notional,
            "max_staleness": cfg.max_staleness,
            "partial_change_threshold": (
                cfg.partial_change_threshold
            ),
            "refill_fill_threshold": (
                cfg.refill_fill_threshold
            ),
            "alpha_enabled": cfg.alpha_enabled,
            "stock_limit_overrides": dict(
                cfg.stock_limit_overrides
            ),
        }
