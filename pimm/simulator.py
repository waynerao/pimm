...
STUB_PRICES = {
    ...
    # CN (CNY)
    "600519.SS": 1700.0, "601318.SS": 50.0, "000858.SZ": 150.0,
    "600036.SS": 35.0, "000333.SZ": 55.0,
    # TW (TWD)
    "2330.TW": 580.0, "2317.TW": 105.0, "2454.TW": 950.0,
    "2412.TW": 120.0, "3711.TW": 280.0,
}
STUB_FX = {"HK": 0.128, "CN": 0.138, "TW": 0.031}
SUFFIX_TO_MARKET = {
    "HK": "HK", "SS": "CN", "SZ": "CN", "TW": "TW",
}
...
STUB_LOT_SIZES = {
    ...
    # CN
    "600519.SS": 100, "601318.SS": 100, "000858.SZ": 100,
    "600036.SS": 100, "000333.SZ": 100,
    # TW
    "2330.TW": 1000, "2317.TW": 1000, "2454.TW": 1000,
    "2412.TW": 1000, "3711.TW": 1000,
}
...
def _get_stub_lot_sizes(ric_list):
    return {
        r: STUB_LOT_SIZES[r]
        for r in ric_list if r in STUB_LOT_SIZES
    }
...
def _sim_risk_appetite(feed, running, rng, all_rics, fx_map):
    while running.is_set():
        ...
        for ric in all_rics:
            ...
            rows.append({
                "ric": ric,
                "buy_state": rng.choice(BUY_PRICE_TYPES),
                "buy_qty": rng.randint(1000, 50000),
                "sell_state": rng.choice(SELL_PRICE_TYPES),
                "sell_qty": rng.randint(1000, 50000),
                "fx_rate": fx,
            })
        df = pd.DataFrame(rows)
        logger.info(f"SIM risk_appetite: {len(df)} rows")
        feed.on_update(df)
        time.sleep(3)
...
def _sim_live_price(feed, running, rng, all_rics, live_prices):
    while running.is_set():
        ...
        df = pd.DataFrame(rows)
        logger.info(f"SIM live_price: {len(df)} rows")
        feed.on_update(df)
        time.sleep(2)
...
def _sim_inventory(feed, running, rng, all_rics):
    while running.is_set():
        rows = [
            {"ric": ric, "inventory": rng.randint(0, 20000)}
            for ric in all_rics
        ]
        df = pd.DataFrame(rows)
        logger.info(f"SIM inventory: {len(df)} rows")
        feed.on_update(df)
        time.sleep(5)
...
def _sim_alpha(feed, running, rng, rics):
    while running.is_set():
        rows = [
            {"ric": ric,
             "alpha": round(rng.uniform(-0.3, 0.3), 4)}
            for ric in rics
        ]
        df = pd.DataFrame(rows)
        logger.info(f"SIM alpha: {len(df)} rows")
        feed.on_update(df)
        time.sleep(20)


def _sim_fills(feed, running, rng, live_quotes,
               live_prices, lot_sizes):
    ...
    while running.is_set():
        ...
        for ric in chosen:
            ...
            base_price = live_prices.get(
                ric, STUB_PRICES.get(ric, 100.0)
            )
        ...
        if rows:
            df = pd.DataFrame(rows)
            logger.info(f"SIM fills: {len(df)} rows")
            feed.on_update(df)
...
def main():
    ...
    for mname, config in all_configs.items():
        ...
        logger.info(
            ...
            f"overrides: sessions=00:00-23:59, "
            f"notional=$500k, full_batch=2min"
        )
    ...
    for mname, ms in market_states.items():
        inv_feed.start_market(mname)
        if ms.config.alpha_enabled:
            alpha_feed.start_market(mname)
    ...
    sim_threads = [
        ...
        threading.Thread(
            ...
            args=(price_feed, running, rng,
                  all_rics, live_prices),
        ...
        threading.Thread(
            target=_sim_fills,
            args=(fills_feed, running, rng, live_quotes,
                  live_prices, STUB_LOT_SIZES),
            name="sim-fills", daemon=True,
        ),
    ]

    for mname, ms in market_states.items():
        if not ms.config.alpha_enabled:
            continue
        rics = list(ms.state_mgr.quotable.index)
        sim_threads.append(threading.Thread(
            target=_sim_alpha,
            args=(alpha_feed, running, rng, rics),
            name=f"sim-alpha-{mname}", daemon=True,
        ))
    ...
    try:
        ...
        loop.close()
        logger.info(
            f"=== pimm simulator stopped === (log: {log_path})"
        )
